...
function createWorkflowService(db, agents) {
  return {
    ...
    /**
     * 一键载入「论文工作流」模板（幂等）：论文 5 阶段串行 pipeline，
     * 每阶段以 JSON 输出 → 节点完成后自动注册为产物（M3）。
     * 结构：大纲 → 初稿 → 自评 → 修订 → 终稿
     */
    async seedPaper() {
      const NAME = '论文工作流模板'
      ...
      const specs = [
        {
          seedKey: 'paper-outline',
          name: '论文·大纲',
          description: '内置：把研究题目拆解为论文大纲',
          agentPersona: '你是学术论文写作专家，擅长把研究主题拆解为结构清晰、逻辑完整的中英文论文大纲（引言、相关工作、方法、实验、结论）。',
          nodePrompt: '请为以下研究题目撰写论文大纲。只输出 JSON（不要其他文字）：\n{"type":"markdown","title":"论文大纲","content":"<markdown 格式的完整大纲>"}\n\n研究题目：\n{{input}}',
        },
        {
          seedKey: 'paper-draft',
          name: '论文·初稿',
          description: '内置：根据大纲撰写论文初稿',
          agentPersona: '你是学术论文初稿作者。你根据论文大纲撰写完整初稿，语言严谨、论述充分、结构清晰。',
          nodePrompt: '请根据以下论文大纲撰写论文完整初稿。只输出 JSON（不要其他文字）：\n{"type":"markdown","title":"论文初稿","content":"<论文完整初稿 body>"}\n\n大纲：\n{{node:paper_outline}}',
        },
        {
          seedKey: 'paper-self-review',
          name: '论文·自评',
          description: '内置：从五维评审论文初稿',
          agentPersona: '你是学术论文评审专家。你从创新性、方法严谨性、实验充分性、写作质量、参考文献五个维度评审论文稿，给出结构化的评分与意见。',
          nodePrompt: '请对以下论文初稿做五维评审。只输出 JSON（不要其他文字）：\n{"type":"table","title":"论文自评表","content":"| 维度 | 评分1-5 | 评价 | 修改建议 |\\n|---|---|---|---|\\n<表格内容>"}\n\n初稿：\n{{node:paper_draft}}',
        },
        {
          seedKey: 'paper-revise',
          name: '论文·修订',
          description: '内置：根据评审意见修订论文',
          agentPersona: '你是学术论文修订专家。你根据评审意见逐条修订论文，处理所有指出的问题，保持行文连贯。',
          nodePrompt: '请根据以下初稿与评审意见修订论文。只输出 JSON（不要其他文字）：\n{"type":"markdown","title":"论文修订稿","content":"<修订后的完整论文 body>"}\n\n初稿：\n{{node:paper_draft}}\n\n评审意见：\n{{node:paper_self_review}}',
        },
        {
          seedKey: 'paper-final',
          name: '论文·终稿',
          description: '内置：整理为最终投稿版本',
          agentPersona: '你是学术论文终稿编辑。你把修订稿整理为格式统一、可直接投稿的最终版本，检查标题层级与引用格式。',
          nodePrompt: '请把以下论文修订稿整理为最终投稿版本。只输出 JSON（不要其他文字）：\n{"type":"markdown","title":"论文终稿","content":"<终稿完整 body>"}\n\n修订稿：\n{{node:paper_revise}}',
        },
      ]
      ...
      // 2. 建模板：5 阶段串行
      const X = 80
      const keys = ['paper_outline', 'paper_draft', 'paper_self_review', 'paper_revise', 'paper_final']
      const nodes = keys.map((key, i) => ({
        key,
        agentId: agentBySeed[specs[i].seedKey].id,
        prompt: specs[i].nodePrompt,
        position: { x: X + i * 300, y: 120 },
      }))
      const edges = keys.slice(0, -1).map((k, i) => ({ source: k, target: keys[i + 1] }))
      return this.create({
        name: NAME,
        description: '论文五阶段流水线：大纲 → 初稿 → 自评 → 修订 → 终稿；每阶段自动产出结构化产物（M3）',
        nodes,
        edges,
      })
    },
